...
def preflight_environment(**values):
 environment = {
...
class TestPreflight(unittest.TestCase):
 def test_uses_passed_missing_provider_configuration_over_a_configured_process(self):
  environment = {'CORDIA_PG_DSN': 'postgres://example', 'CORDIA_VAULT_KEY': 'key',
                 'CORDIA_DEV_2FA': '1'}
  with patch.dict(os.environ, preflight_environment(), clear=True):
  ...
  self.assertFalse(result['checks']['model_provider']['configured'])

 def test_uses_passed_configured_provider_configuration_over_a_missing_process(self):
  with patch.dict(os.environ, {}, clear=True):
   result = preflight.report(preflight_environment(CORDIA_PG_DSN='postgres://example',
                                                    CORDIA_VAULT_KEY='key', CORDIA_DEV_2FA='1'),
                             has_cryptography=True, has_psycopg2=True, database_ready=True,
                             dependency_available=lambda _module: True)
  self.assertTrue(result['checks']['model_provider']['configured'])
 ...
 def test_reports_configured_openai_provider_as_ready_without_calling_it(self):
  environment = preflight_environment(CORDIA_PG_DSN='postgres://example',
                                      CORDIA_VAULT_KEY='key', CORDIA_DEV_2FA='1')
 ...
 def test_reports_missing_live_requirements_without_reading_values(self):
  result = preflight.report(preflight_environment(), has_cryptography=True, has_psycopg2=True,
                            dependency_available=lambda _module: True)
  self.assertFalse(result['ok'])
 ...
 def test_reports_optional_embedding_dependencies_without_blocking_readiness(self):
  result = preflight.report(preflight_environment(CORDIA_PG_DSN='postgres://example',
                                                   CORDIA_VAULT_KEY='key', CORDIA_DEV_2FA='1'),
                            has_cryptography=True, has_psycopg2=True,
                            database_ready=True,
 ...
 def test_reports_an_unreachable_database_without_disclosing_the_dsn(self):
  result = preflight.report(preflight_environment(CORDIA_PG_DSN='postgres://secret-host',
                                                   CORDIA_VAULT_KEY='key', CORDIA_DEV_2FA='1'),
                            has_cryptography=True, has_psycopg2=True,
