...
async function render(component, link) {
  ...
  try {
    await act(async () => {
      renderer = TestRenderer.create(React.createElement(component, { link }))
    })
    return renderer
  } finally {
...
test('GitHub artifact renders only the fixed same-origin repository route', async () => {
  ...
  const renderer = await render(linkModule.default, safeLink)
  try {
